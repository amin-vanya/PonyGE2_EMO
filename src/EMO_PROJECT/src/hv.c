...
/*
 * Setup circular double-linked list in each dimension.
 * It considers only enable solutions. (RHG)
 */
...
/* RHG double fpli_hv(double *data, int d, int n, const double *ref) */
/*double hv(double *data, int n, const double *ref, int d) */
double EMO_hv2(double *data, int n, const double *ref, int d)
{
    ...
    if(n2 != n)
      printf("Warning: omitting %d/%d points that are outside the reference point.\n", n2-n, n2);
...
/* Hipervolume contribution, RHG */
/*double EMO_hv_contribution(double *data, double *deltahv, int n, int d, const double *ref) {
  double hv;
  int i;

  totalhv = EMO_hv(data, n, d, ref);

  for(i = 0; i < n; i++) { 
    enable[i] = 0;
    deltahv[i] = totalhv - EMO_hv(data, enable, n, d, ref);
    enable[i] = 1;
  }
