/* matrix.c:   Programa que calcula la matriz inversa de una matriz.
 *             Utiliza descomposicion LU o Cholesky.
 *
 * Autor:                   Raquel Hernandez Gomez 
 * Materia:                 Topicos Selectos de Computacion Cientifica I
 * Profesor:                Dr. Amilcar Meneses Viveros 
 * 
 * Mexico D.F, CINVESTAV-IPN 
 * Julio 2014 
 */
...
int lu(double *L, double *U, double *A, int n, int nthread) {
  ...
  //#pragma omp parallel shared(L,U,A,n,nthread,debug,size,ntask,ktask,task,k,r) private(h,i,j,u)
  //{
    //h = omp_get_thread_num();
    h = 0; 

    while(k < size && r == 0) {
      //RHG printf("k %d, h %d, nthread %d\n", k, h, nthread);
      ...
      while(task < ktask) {
        ...
        if(i >= 0 && i < n && j >= 0 && j < n) {
          ...
          if(debug) 
            printf("Tarea %d, hilo %d: m[%d,%d]\n", task, h, i, j);
        }

        /* Sincronizacion de todos los hilos */
        //#pragma omp barrier

        /* Un unico hilo incrementa contador de tareas */
        //#pragma omp single
        task++;
      }
...
int cholesky(double *L, double *Lt, double *A, int n, int nthread) {
  ...
  //#pragma omp parallel shared(L,Lt,A,n,nthread,debug,size,ntask,ktask,task,k, r) private(h,i,j,u,sum)
  //{
    //h = omp_get_thread_num();
    h = 0;
...
int forwardsubstitution(double *y, double *L, double *b, int n) {
  ...
  for(i = 0; i < n; i++) {
    ...
    if(L[i*n+i] == 0) { printf("matrix:forwardsubstitution:error\n");
      return 1; }
...
int backsubstitution(double *x, double *U, double *y, int n) {
  ...
  for(i = n-1; i >= 0; i--) {
    ...
    if(U[i*n+i] == 0) { printf("matrix:backsubstitution:error\n");
      return 1; }
...
int EMO_minverse(double *B, double *A, int n, int nthread, int metodo, int tridiag) {
  ...
  if(debug) EMO_mprint(stdout, L, n, n, "Factorizacion L");
  if(debug) EMO_mprint(stdout, U, n, n, "Factorizacion U");

  //#pragma omp parallel private(i,j,b,y,a,str) shared(L,U,B,n,r) 
  //{
    ...
    for(i = 0; i < n; i++) {
      ...
      if(r == 0) {
        ...
        if(forwardsubstitution(y, L, b, n) == 1) { // RHG printf("-------error\n");
          //#pragma omp atomic
          r++;
        }

        if(backsubstitution(a, U, y, n) == 1) { // RHG printf("-------error\n");
          //#pragma omp atomic
          r++;
        }
