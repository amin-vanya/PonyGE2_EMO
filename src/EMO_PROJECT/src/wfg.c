...
/* 
  ...
  Carlos A. Coello Coello, Arturo Hernández Aguirre, 
  and Eckart Zitzler (Eds.). 
...
double EMO_WFG_sh_linear_1(double *x, int M) {
  ...
  return EMO_WFG_correct_to_01(v);
}
...
double EMO_WFG_sh_linear_m(double *x, int M, int m) {
  ...
  return EMO_WFG_correct_to_01(v * (1.0 - x[M-m]));
}

double EMO_WFG_sh_linear_M(double *x) {
  return EMO_WFG_correct_to_01(1.0 - x[0]);
}

void EMO_WFG_sh_linear(double *h, double *x, int M) {
  int i;

  h[0] = EMO_WFG_sh_linear_1(x, M);

  for(i = M - 2; i > 0; i--)
    h[i] = EMO_WFG_sh_linear_m(x, M, i + 1);

  h[M-1] = EMO_WFG_sh_linear_M(x);
}
...
double EMO_WFG_sh_convex_m(double *x, int M, int m) {
  ...
  return EMO_WFG_correct_to_01(v * (1.0 - sin(x[M-m] * PI * 0.5)));
}

double EMO_WFG_sh_convex_M(double *x) {
  return EMO_WFG_correct_to_01(1.0 - sin(x[0] * PI * 0.5));
}

void EMO_WFG_sh_convex(double *h, double *x, int M) {
  int i;

  h[0] = EMO_WFG_sh_convex_1(x, M);

  for(i = M - 2; i > 0; i--)
    h[i] = EMO_WFG_sh_convex_m(x, M, i + 1);

  h[M-1] = EMO_WFG_sh_convex_M(x);
}
...
double EMO_WFG_sh_concave_m(double *x, int M, int m) {
  ...
  return EMO_WFG_correct_to_01(v * cos(x[M-m] * PI * 0.5));
}

double EMO_WFG_sh_concave_M(double *x) {
  return EMO_WFG_correct_to_01(cos(x[0] * PI * 0.5));
}

void EMO_WFG_sh_concave(double *h, double *x, int M) {
  int i;

  h[0] = EMO_WFG_sh_concave_1(x, M);

  for(i = M - 2; i > 0; i--) 
    h[i] = EMO_WFG_sh_concave_m(x, M, i + 1);
 
  h[M-1] = EMO_WFG_sh_concave_M(x);
}
...
double EMO_WFG_sh_mixed_M(double *x, double A, double alpha) {
  ...
  v /= 2.0 * A * PI;
  v = 1.0 - x[0] - v;
  return EMO_WFG_correct_to_01(pow(v, alpha));
}
...
double EMO_WFG_sh_disc_M(double *x, double A, double alpha, double beta) {
  ...
  return EMO_WFG_correct_to_01(1.0 - v);
}
...
double EMO_WFG_tr_b_poly(double y, double alpha) {
  return EMO_WFG_correct_to_01(pow(y, alpha));
}
...
double EMO_WFG_tr_b_flat(double y, double A, double B, double C) {
  ...
  return EMO_WFG_correct_to_01(A + v - w);
}
...
double EMO_WFG_tr_b_param(double y, double u, double A, double B, double C) {
  ...
  return EMO_WFG_correct_to_01(pow(y, v));
}
...
double EMO_WFG_tr_s_linear(double y, double A) {
  ...
  v = A - y;
  return EMO_WFG_correct_to_01(fabs(v) / fabs(floor(v) + A));
}
...
double EMO_WFG_tr_s_decept(double y, double A, double B, double C) {
  ...
  return EMO_WFG_correct_to_01(1.0 + v4);
}
...
double EMO_WFG_tr_s_multi(double y, double A, double B, double C) {
  ...
  return EMO_WFG_correct_to_01(v5 / (B + 2.0));
}
...
double EMO_WFG_tr_r_sum(double *y, double *w, int n) {
  ...
  return EMO_WFG_correct_to_01(v1 / v2);
}
...
double EMO_WFG_tr_r_nonsep(double *y, double A, int n) {
  ...
  return EMO_WFG_correct_to_01(v2 / (n / A * v3 * (1.0 + 2.0 * A - 2.0 * v3)));
}
...
void EMO_WFG_wfg1_t1(EMO_MOP *mop, double *z) {
  ...
  for(i = mop->npos; i < mop->nvar; i++) 
    mop->t[i] = EMO_WFG_tr_s_linear(z[i], 0.35);
}
...
void EMO_WFG_wfg1_t2(EMO_MOP *mop, double *z) {
  ...
  for(i = mop->npos; i < mop->nvar; i++) 
    mop->t[i] = EMO_WFG_tr_b_flat(z[i], 0.8, 0.75, 0.85); 
}
...
void EMO_WFG_wfg1_t3(EMO_MOP *mop, double *z) {
  ...
  for(i = 0; i < mop->nvar; i++) 
    mop->t[i] = EMO_WFG_tr_b_poly(z[i], 0.02);
}
...
void EMO_WFG_wfg1_t4(EMO_MOP *mop, double *z) {
  ...
  for(i = 1; i <= f; i++) {
    ...
    mop->t[i-1] = EMO_WFG_tr_r_sum(mop->y, mop->w, idx); 
  }
  ...
  mop->t[mop->nobj-1] = EMO_WFG_tr_r_sum(mop->y, mop->w, idx); 
}
...
void EMO_WFG_wfg2_t2(EMO_MOP *mop, double *z) {
  ...
  for(i = mop->npos + 1; i <= f; i++) {
    ...
    mop->t[i-1] = EMO_WFG_tr_r_nonsep(mop->y, 2.0, idx);
  }
}
...
void EMO_WFG_wfg4_t1(EMO_MOP *mop, double *z) {
  ...
  for(i = 0; i < mop->nvar; i++) 
    mop->t[i] = EMO_WFG_tr_s_multi(z[i], 30.0, 10.0, 0.35); 
}
...
void EMO_WFG_wfg4_t2(EMO_MOP *mop, double *z) {
  ...
  for(j = mop->npos; j < mop->nvar; j++) { /* modified from EMO_WFG_wfg2_t3 */
    mop->w[idx] = 1.0;
    mop->y[idx] = z[j];
    idx++;
  } 
...
void EMO_WFG_wfg5_t1(EMO_MOP *mop, double *z) {
  ...
  for(i = 0; i < mop->nvar; i++) 
    mop->t[i] = EMO_WFG_tr_s_decept(z[i], 0.35, 0.001, 0.05);
} 
...
void EMO_WFG_wfg6_t2(EMO_MOP *mop, double *z) {
  ...
  for(i = 1; i <= f; i++) {
    ...
    mop->t[i-1] = EMO_WFG_tr_r_nonsep(mop->y, (double) mop->npos / (mop->nobj - 1.0), idx); 
  }
  ...
  mop->t[mop->nobj-1] = EMO_WFG_tr_r_nonsep(mop->y, mop->nvar-mop->npos, idx); 
} 
...
void EMO_WFG_wfg7_t1(EMO_MOP *mop, double *z) {
  ...
  for(i = 0; i < mop->npos; i++) { 
    ...
    mop->t[i] = EMO_WFG_tr_b_param(z[i], EMO_WFG_tr_r_sum(mop->y, mop->w, idx), 0.98/49.98, 0.02, 50.0); 
  }
...
void EMO_WFG_wfg9_t2(EMO_MOP *mop, double *z) {
  ...
  for(i = mop->npos; i < mop->nvar; i++) 
    mop->t[i] = EMO_WFG_tr_s_multi(z[i], 30.0, 95.0, 0.35); 
}
...
void EMO_WFG_wfg1(EMO_MOP *mop, double *f, double *z) {
  ...
  EMO_WFG_wfg1_t1(mop, z);      /* Transformation t1 */
  EMO_WFG_wfg1_t2(mop, mop->t); /* Transformation t2 */
  EMO_WFG_wfg1_t3(mop, mop->t); /* Transformation t3 */
  EMO_WFG_wfg1_t4(mop, mop->t); /* Transformation t4 */

  EMO_WFG_calc_x(mop->x, mop->t, mop->A, mop->nobj); /* Vector x */ 
 
  /* Shape */ 
  mop->h[0] = EMO_WFG_sh_convex_1(mop->x, mop->nobj);

  for(i = mop->nobj - 2; i > 0; i--) 
    mop->h[i] = EMO_WFG_sh_convex_m(mop->x, mop->nobj, i + 1);
 
  mop->h[mop->nobj-1] = EMO_WFG_sh_mixed_M(mop->x, 5.0, 1.0);

  EMO_WFG_calc_f(f, mop->x, mop->S, mop->h, mop->nobj); /* Objective functions */ 
}
...
void EMO_WFG_wfg2(EMO_MOP *mop, double *f, double *z) {
  ...
  EMO_WFG_wfg1_t1(mop, z);      /* Transformation t1 */
  EMO_WFG_wfg2_t2(mop, mop->t); /* Transformation t2 */
  EMO_WFG_wfg2_t3(mop, mop->t); /* Transformation t3 */
  ...
  mop->h[mop->nobj-1] = EMO_WFG_sh_disc_M(mop->x, 5.0, 1.0, 1.0);
...
void EMO_WFG_wfg3(EMO_MOP *mop, double *f, double *z) {
  ...
  EMO_WFG_wfg2_t3(mop, mop->t);                         /* Transformation t3 */
  EMO_WFG_calc_x(mop->x, mop->t, mop->A, mop->nobj);    /* Vector x */ 
  EMO_WFG_sh_linear(mop->h, mop->x, mop->nobj);         /* Shape */ 
  EMO_WFG_calc_f(f, mop->x, mop->S, mop->h, mop->nobj); /* Objective functions */ 
}

void EMO_WFG_wfg4(EMO_MOP *mop, double *f, double *z) {
  EMO_WFG_wfg4_t1(mop, z);                              /* Transformation t1 */
  EMO_WFG_wfg4_t2(mop, mop->t);                         /* Transformation t2 */
  EMO_WFG_calc_x(mop->x, mop->t, mop->A, mop->nobj);    /* Vector x */ 
  EMO_WFG_sh_concave(mop->h, mop->x, mop->nobj);        /* Shape */ 
  EMO_WFG_calc_f(f, mop->x, mop->S, mop->h, mop->nobj); /* Objective functions */ 
}

void EMO_WFG_wfg5(EMO_MOP *mop, double *f, double *z) {
  EMO_WFG_wfg5_t1(mop, z);                              /* Transformation t1 */
  EMO_WFG_wfg4_t2(mop, mop->t);                         /* Transformation t2 */
  EMO_WFG_calc_x(mop->x, mop->t, mop->A, mop->nobj);    /* Vector x */ 
  EMO_WFG_sh_concave(mop->h, mop->x, mop->nobj);        /* Shape */ 
  EMO_WFG_calc_f(f, mop->x, mop->S, mop->h, mop->nobj); /* Objective functions */ 
...
void EMO_WFG_wfg6(EMO_MOP *mop, double *f, double *z) {
  EMO_WFG_wfg1_t1(mop, z);                               /* Transformation t1 */
  EMO_WFG_wfg6_t2(mop, mop->t);                          /* Transformation t2 */
  EMO_WFG_calc_x(mop->x, mop->t, mop->A, mop->nobj);     /* Vector x */ 
  EMO_WFG_sh_concave(mop->h, mop->x, mop->nobj);         /* Shape */ 
  EMO_WFG_calc_f(f, mop->x, mop->S, mop->h, mop->nobj);  /* Objective functions */ 
}
...
void EMO_WFG_wfg7(EMO_MOP *mop, double *f, double *z) {
  EMO_WFG_wfg7_t1(mop, z);                              /* Transformation t1 */
  EMO_WFG_wfg1_t1(mop, mop->t);                         /* Transformation t2 */
  EMO_WFG_wfg4_t2(mop, mop->t);                         /* Transformation t3 */
  EMO_WFG_calc_x(mop->x, mop->t, mop->A, mop->nobj);    /* Vector x */ 
  EMO_WFG_sh_concave(mop->h, mop->x, mop->nobj);        /* Shape */ 
  EMO_WFG_calc_f(f, mop->x, mop->S, mop->h, mop->nobj); /* Objective functions */ 
}
...
void EMO_WFG_wfg8(EMO_MOP *mop, double *f, double *z) {
  EMO_WFG_wfg8_t1(mop, z);                              /* Transformation t1 */
  EMO_WFG_wfg1_t1(mop, mop->t);                         /* Transformation t2 */
  EMO_WFG_wfg4_t2(mop, mop->t);                         /* Transformation t3 */
  EMO_WFG_calc_x(mop->x, mop->t, mop->A, mop->nobj);    /* Vector x */ 
  EMO_WFG_sh_concave(mop->h, mop->x, mop->nobj);        /* Shape */ 
...
void EMO_WFG_wfg9(EMO_MOP *mop, double *f, double *z) {
  EMO_WFG_wfg9_t1(mop, z);                              /* Transformation t1 */
  EMO_WFG_wfg9_t2(mop, mop->t);                         /* Transformation t2 */
  EMO_WFG_wfg6_t2(mop, mop->t);                         /* Transformation t3 */
  EMO_WFG_calc_x(mop->x, mop->t, mop->A, mop->nobj);    /* Vector x */ 
  EMO_WFG_sh_concave(mop->h, mop->x, mop->nobj);        /* Shape */ 
  EMO_WFG_calc_f(f, mop->x, mop->S, mop->h, mop->nobj); /* Objective functions */ 
}
...
void EMO_WFG_wfg9_minus(EMO_MOP *mop, double *f, double *z) {
  ...
  EMO_WFG_wfg9(mop, f, z);

  for(i = mop->nobj - 1; i > -1; i--)
    f[i] *= -1.0;
}

/*void wfg1_sol(double *f, double *x) {
  int i;

  for(i = 0; i < mop->npos; i++) 
    if(EMO_flip(0.3))
      x[i] = EMO_rndreal(1e-10, 1.0);
    else if(EMO_flip(0.35))
      x[i] = EMO_rndreal(1e-20, 1e-10);
    else if(EMO_flip(0.40))
      x[i] = EMO_rndreal(1e-30, 1e-20);
    else if(EMO_flip(0.45))
      x[i] = EMO_rndreal(1e-40, 1e-30);
    else if(EMO_flip(0.50))
      x[i] = EMO_rndreal(1e-50, 1e-40);
    else
      x[i] = EMO_rndreal(0, 1e-50);
...
/*void wfg2_sol(double *f, double *x) {
  int i;

  for(i = 0; i < mop->npos; i++) 
    if(EMO_flip(0.5))
      x[i] = EMO_rndreal(0.98, 1);
    else if(EMO_flip(0.55))
      x[i] = EMO_rndreal(1e-4,0.98);
    else
      x[i] = EMO_rndreal(0,0.1e-4);
...
/*void wfg3_sol(double *f, double *x) {
  int i;
