...
class HVC_10D_StFt(base_ff):
    ...
    def evaluate(self, ind, **kwargs):
        ...
        for file_index in range(self.n_files):
            ...
            mu = self.mu[file_index]
            sig = self.sig[file_index]
            gamma = self.gamma[file_index]
            kappa = self.kappa[file_index]
            Q1 = self.Q1[file_index]
            Q2 = self.Q2[file_index]
            Q3 = self.Q3[file_index]
    ...
    def get_data(self):
        ...
        self.mu = np.zeros((self.n_files,self.dim))
        self.sig = np.zeros((self.n_files,self.dim))
        self.gamma = np.zeros((self.n_files,self.dim))
        self.kappa = np.zeros((self.n_files,self.dim))
        self.Q1 = np.zeros((self.n_files,self.dim))
        self.Q2 = np.zeros((self.n_files,self.dim))
        self.Q3 = np.zeros((self.n_files,self.dim))
        ...
        for file_index in range(self.n_files): # Recorre los archivos
            ...
            # obtain statistic features
            self.mu[file_index] = np.mean(self.population[file_index],0)
            self.sig[file_index] = np.std(self.population[file_index],0)
            self.gamma[file_index] = skew(self.population[file_index], axis=0, bias=True)
            self.kappa[file_index] = kurtosis(self.population[file_index], axis=0, fisher=False, bias=True)
            self.Q1[file_index] = np.quantile(self.population[file_index], 0.25, axis=0)
            self.Q2[file_index] = np.quantile(self.population[file_index], 0.5, axis=0)
            self.Q3[file_index] = np.quantile(self.population[file_index], 0.75, axis=0)
